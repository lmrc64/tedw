...
export default function UpdateUserForm({
  ...
  useEffect(() => {
    setFormData(initialData); // Update state when initialData changes
  }, [initialData]);
  ...
  return (
      <section className="bg-white px-4 py-6 rounded-md font-primary font-extrabold">
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
          ...
          {renderInput("Phone", "phone", "text", formData.phone, "1234567890")}
          <CountrySelect
              onChange={(country) => setFormData((prev) => ({ ...prev, country }))}
              value={formData.country}
          />
          {renderInput("State", "state", "text", formData.state, "e.g. Guanajuato")}
          {renderInput("City", "city", "text", formData.city, "e.g. Celaya")}
          {renderInput("ZIP Code", "zip", "text", formData.zip, "e.g. 38080")}
